...
public class ReLogoPatch extends BasePatch{
	...
	/**
	 * Sprouts (makes) a number of new roundabouts and then executes a set of commands on the
	 * created roundabouts.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<traffic.relogo.Roundabout> sproutRoundabouts(int number, Closure closure) {
		AgentSet<traffic.relogo.Roundabout> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.sprout(number,closure,"Roundabout");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("traffic.relogo.Roundabout")
	public AgentSet<traffic.relogo.Roundabout> sproutRoundabouts(int number) {
		return sproutRoundabouts(number,null);
	}
