...
### Porównanie wyników
```{r compare}
results = results(model=c('lm', 'knn', 'lasso'), r2=c(r2s$lm, r2s$knn, r2s$lasso), rmse=c(rmses$lm, rmses$knn, rmses$lasso))
kable(results)
...
```
Regresor k-NN, w którym za k przyjęto ?, popełnił najmniejszy błąd i wyjaśnił najwięcej zmienności danych. Z drugiej strony, strojenie go trwało najdłużej, gdyż predykcja z tego modelu wymaga przejrzenia wszystkich zapamiętanych przykładów.
